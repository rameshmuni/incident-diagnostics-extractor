...
# written the way a person would actually describe the symptom, not the fix.
CATALOG = [
    {
        "issue_id": "login_failure",
        "short_description": (
            "User or service account cannot log in to the application - login repeatedly "
            "fails, the account appears locked, or credentials are rejected."
        ),
        "remedy": remedies.fix_login_failure,
    },
    {
        "issue_id": "stuck_queue",
        "short_description": (
            "A queue item or automation job is stuck In Progress and not completing, "
            "blocking downstream items in the queue from being processed."
        ),
        "remedy": remedies.fix_stuck_queue,
    },
    {
        "issue_id": "expired_credential",
        "short_description": (
            "An API token, certificate, or service credential has expired, causing "
            "authentication failures when connecting to a downstream system."
        ),
        "remedy": remedies.fix_expired_credential,
    },
    {
        "issue_id": "disk_full",
        "short_description": (
            "The application server or robot machine is low on disk space, causing "
            "failures writing logs or temporary files."
        ),
        "remedy": remedies.fix_disk_full,
    },
]
...
def _create_and_resolve_incident(new_incident_text, resolution_text):
    # mirrors create_incident()/resolve_incident() in seed_resolved_incidents.py, collapsed
    # into one call: open the ticket, then immediately close it with the remedy's own
    # Root Cause + Resolution text - this is the actual "update the same resolution in the
    # incident" step the manager asked for
    ...
    incident = _parse_json(resp)["result"]
    ...
    resp2 = _check(requests.patch(
        f"{BASE_URL}/table/incident/{incident['sys_id']}", auth=AUTH, headers=HEADERS, timeout=30, json=resolve_payload
    ))
    return _parse_json(resp2)["result"]


def attempt_auto_remediation(new_incident_text):
    """
    The one function app.py calls when human-in-the-loop is off. Always returns a dict
    describing exactly what happened - there's no "pending" state coming out of this, unlike
    the Week 2 flow: by the time this returns, the incident has already been either resolved
    or escalated in ServiceNow.
    """
    ...
    incident = _create_and_resolve_incident(new_incident_text, resolution_text)
    return {
